...
<div class="d-flex justify-content-between align-items-center mb-4" style="padding-right: 10px;">
    ...
    <a href="{{ route('error_trackings.create') }}" class="btn btn-primary ml-auto">Add New Error</a>
</div>
        ...
        <tbody>
            @forelse ($errorTrackings as $errorTracking)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('error_trackings.edit', $errorTracking->id) }}" class="btn btn-warning btn-sm">Edit</a>
                        <a href="{{ route('error_trackings.show', $errorTracking->id) }}" class="btn btn-info btn-sm">View</a>
                        <form action="{{ route('error_trackings.destroy', $errorTracking->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this error tracking record?')">Delete</button>
                        </form>
                    </td>
                </tr>
            @empty
